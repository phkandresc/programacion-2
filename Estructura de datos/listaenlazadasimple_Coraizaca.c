...
nodo *eliminarNodo(nodo *head)
{
    nodo *nodoaEliminar = buscarNodo(head);
    ...
    // Busca el nodo que precede al nodo a eliminar
    nodo *prev = head;
    while (prev->sig != NULL && prev->sig->valor != valor)
    {
        prev = prev->sig;
    }

    // Si se encontró el nodo, elimínalo
    if (prev->sig != NULL)
    {
        nodo *temp = prev->sig;
